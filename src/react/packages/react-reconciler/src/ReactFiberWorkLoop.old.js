...
export function requestUpdateLane(fiber: Fiber): Lane {
  /** @desc Special cases 首先检查fiber的mode。如果不是ConcurrentMode（即不支持并发模式），则直接返回SyncLane，表示这个更新应该同步执行。 */
  const mode = fiber.mode;
  if ((mode & ConcurrentMode) === NoMode) {
    return (SyncLane: Lane);
  } else if (
    /** @desc 渲染阶段更新：如果当前不是在延迟渲染阶段更新到下一个批次，并且当前执行上下文是渲染上下文，并且有正在进行的渲染工作，则将更新分配给当前正在渲染的Lane。 */
    !deferRenderPhaseUpdateToNextBatch &&
  ...
  /** @desc 处理过渡（Transition）：如果当前更新是在一个过渡（Transition）中发生的，会为这个过渡分配一个特定的Lane。所有在同一个事件中的过渡共享同一个Lane。 */
  ...
  /** @desc **根据当前更新优先级分配Lane**：如果当前有通过特定React方法（如flushSync）设置的优先级，直接使用这个优先级作为Lane。 */
...
function markUpdateLaneFromFiberToRoot(
  ...
  console.log("【解释】markUpdateLaneFromFiberToRoot内部会递归，检测fiber的parent");
  while (parent !== null) {
...
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  ...
  0代表NoLanes表示没任务需要处理，
  1代表同步最高优先级（同步调度），
  4代表处理哪些用户输入等触发的更新的优先级，
  16代表默认优先级比如初次render等。
  发现下一个更新优先级为0时候，就会检测是否存在root.callbackNode，如果存在就会调度执行（root.callbackNode就是那些被高优先级任务中断的低优先级任务）。
  若下一更新优先级不为0，就会使用优先级最高的通道来表示下一回调的优先级，然后获取当前根节点上已存在回调的优先级，如果发现已存在优先级和下一优先级一样就重用，现在的任务不会再向synqueue里面增加任务。
...
function workLoopSync() {
  ...
  console.log('【初次渲染2】zono1.初次render或者任务过期，都会调用同步的workLoop更新，不会进行时间切片，都是调用workLoopSync,不管是同步还是异步都会while循环调用performUnitOfWork')
  console.log(`【打印】workLoop主要是分三个阶段：
  1、是beginWork，从hostRootFiber向下递归创建或者复用fiber给fiber打上tag（这个tag就是对应的增删改）。
  2、通过completeWork向上递归到Host处理props创建dom和effectlist等，这个effectlist就是收集那些打上tag的fiber，方便在commit阶段直接遍历这个list执行更改
  3、在commit阶段提交突变，遍历effectlist（增删改对应的fiber），执行effect`)
...
function commitRootImpl(
  ...
  do {
    ...
    // 这个函数很重要 清除effect
    console.log('【提交阶段】zono2.先执行flushPassiveEffects执行effect任务，flushPassiveEffects这个函数里面会先执行effect的清除函数再执行effect')
    flushPassiveEffects();
...
function scheduleCallback(priorityLevel, callback) {
  if (__DEV__) {
    ...
    if (actQueue !== null) {
      ...
      console.log('【解释】调用调度更新，将会调用schedule包中的scheduleCallback函数');
...
function cancelCallback(callbackNode) {
  ...
  // In production, always call Scheduler. This function will be stripped out.（剥离出来）
